...
// 					$this->api_created = false;
// 				}
				if (!file_exists($api_conf_file) ) {
					$this->api_last_error = " 1 Unable to find/read the YAML api_conf_file : $api_conf_file";
					$this->api_created = false;
				}
				elseif (!is_file($api_conf_file) ) {
					$this->api_last_error = "2 Unable to find/read the YAML api_conf_file : $api_conf_file";
					$this->api_created = false;
				}
				elseif (!is_readable($api_conf_file)) {
					$this->api_last_error = "3 Unable to find/read the YAML api_conf_file : $api_conf_file";
					$this->api_created = false;
				}
		...
		elseif (!$type || !$this->api_conf->Interfaces->$name->$type) {
			...
			if ($this->requestRedirectUrl != null) {
				...
				elseif (in_array($responsecode, array('201','204','301','304'))) {
					$this->api_request_log[] = array('name' => $name, 'type' => $type, 'result' => $responsecode, 'time' => microtime(true)-$orig);
					return true;
				}
				else {
